 ...
 */
public class FileSenderPrivate extends SwingWorker<List<String>,String>{
    private File file; //文件
    private Message msg;//消息类
    private ServerUI serverUI; //父类
    ...
    public FileSenderPrivate(File file, Message msg, ServerUI serverUI) {
    ...
    protected List<String> doInBackground() throws Exception {  
        //用客户机密钥库初始化SSL传输框架
        InputStream key =ServerUI.class.getResourceAsStream("/cn/edu/ldu/keystore/client.keystore");//私钥库
        InputStream tkey =ServerUI.class.getResourceAsStream("/cn/edu/ldu/keystore/tclient.keystore");//公钥库
        ...
        //用SSLSocket连接服务器
        fileSocket = (SSLSocket)ctx.getSocketFactory().createSocket(msg.getAddrString(),msg.getPort());
        //构建套接字输出流
        ...
        out.flush();
        serverUI.txtArea.append("1.发送文件名称、文件长度成功！\n");
        //2.传送文件内容
        ...
        in.close();
        din.close();
        serverUI.txtArea.append("2.传送文件内容成功！\n");
        byte[] fileDigest=in.getMessageDigest().digest(); //生成文件摘要
        serverUI.txtArea.append("生成的摘要："+DatatypeConverter.printHexBinary(fileDigest)+"\n\n"); 
        //用私钥对摘要加密，形成文件的数字签名
        ...
        //更新显示
        serverUI.txtArea.append("生成的数字签名："+DatatypeConverter.printHexBinary(signature)+"\n\n"); 
        
        //生成AES对称密钥
        SecretKey secretKey=Cryptography.generateNewKey();
       serverUI.txtArea.append("生成的密钥："+DatatypeConverter.printHexBinary(secretKey.getEncoded())+"\n\n"); 
        
        //对数字签名加密
        Cipher cipher2=Cipher.getInstance("AES");
        cipher2.init(Cipher.ENCRYPT_MODE, secretKey);//初始化加密器
        byte[] encryptSign=cipher2.doFinal(signature);//生成加密签名
        serverUI.txtArea.append("用密钥加密后的数字签名："+DatatypeConverter.printHexBinary(encryptSign)+"\n\n");         
        //对密钥加密
        cipher.init(Cipher.ENCRYPT_MODE, publicKey);//用服务器公钥初始化加密模式
        byte[] encryptKey=cipher.doFinal(secretKey.getEncoded());//加密密钥 
        serverUI.txtArea.append("对密钥加密："+DatatypeConverter.printHexBinary(encryptKey)+"\n\n");
        ...
        out.flush();
        serverUI.txtArea.append("3.发送加密的数字签名成功！\n");
        
        //4.发送加密密钥
        out.write(encryptKey);//密文长度为128字节
        out.flush();
       serverUI.txtArea.append("4.发送加密的密钥成功！\n");
        ...
        if (response.equalsIgnoreCase("M_DONE")) { //服务器成功接收               
            lastResults= "5."+ file.getName() +"  你的朋友成功接收！\n" ;
        }else if (response.equalsIgnoreCase("M_LOST")){ //服务器接收失败
            lastResults= "5."+ file.getName() +" 你的朋友接收失败！\n" ;
        }//end if
        //关闭流
        br.close();
        out.close();
   ...
   /* protected void process(List<String> middleResults) {
        for (String str:middleResults) {
           serverUI.progressLabel.setText(str);
        }   
    }*/
    /*@Override
    protected void done() {
        serverUI.progressBar.setValue(privateClientUI.progressBar.getMaximum());
       serverUI.txtArea.append(lastResults+"\n");
        privateClientUI.filePanel.setVisible(false);
    }*/
}