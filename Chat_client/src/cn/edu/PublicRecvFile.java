 ...
 */
//***********xfxf 变成extends SwingWorker<List<String>,String>仿照FileSender
public class PublicRecvFile extends SwingWorker<List<String>,String> {
    ...
    public PublicRecvFile(SSLSocket fileSocket,ClientUI clientUI,KeyStore tks,KeyStore ks) { 
    ...
    protected List<String> doInBackground() throws Exception {        
        ...
        //************xfxf显示进度条设置文件名**********
        clientUI.fileLabel.setText("文件:"+filename);
        clientUI.filePanel.setVisible(true);
        ...
        while (numFinished<fileLen && (numRead=in.read(buffer))!=-1) { //输入流可读      
           ...
           // System.out.println("bbbbbbnumFinished:"+numFinished);
...
//更新显示
        ...
        pw.close();
        fileSocket.close();
        //*************xfxf***删除return 100， 加上 return null, 仿照FileSender 这个函数的返回值是List<String>，所以返回100编译错误*******
        //return 100;
        return null;
    }//end doInBackground 
    ...
    protected void process(List<String> middleResults) {
        for (String str:middleResults) {
        	clientUI.progressLabel.setText(str);
        }   
    }
    @Override
    protected void done() {
    	    clientUI.progressBar.setValue(clientUI.progressBar.getMaximum());
    	    //clientUI.txtArea.append(lastResults+"\n");
    	    clientUI.filePanel.setVisible(false);
    }
