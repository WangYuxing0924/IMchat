...
public class FileSenderPrivate extends SwingWorker<List<String>,String>{
    ...
    private PrivateClientUI privateClientUI; //父类
    ...
    public FileSenderPrivate(File file, Message msg, PrivateClientUI privateClientUI) {
		// TODO Auto-generated constructor stub
    	this.file=file;
        this.msg=msg;
        this.privateClientUI=privateClientUI;
	}
	@Override
    protected List<String> doInBackground() throws Exception {  
        ...
        out.flush();
        privateClientUI.txtArea.append("1.发送文件名称、文件长度成功！\n");
        //2.传送文件内容
        ...
        in.close();
        din.close();
        privateClientUI.txtArea.append("2.传送文件内容成功！\n");
        byte[] fileDigest=in.getMessageDigest().digest(); //生成文件摘要
        privateClientUI.txtArea.append("生成的摘要："+DatatypeConverter.printHexBinary(fileDigest)+"\n\n"); 
        //用私钥对摘要加密，形成文件的数字签名
        ...
        //更新显示
        privateClientUI.txtArea.append("生成的数字签名："+DatatypeConverter.printHexBinary(signature)+"\n\n"); 
        
        //生成AES对称密钥
        SecretKey secretKey=Cryptography.generateNewKey();
        privateClientUI.txtArea.append("生成的密钥："+DatatypeConverter.printHexBinary(secretKey.getEncoded())+"\n\n"); 
        
        //对数字签名加密
        Cipher cipher2=Cipher.getInstance("AES");
        cipher2.init(Cipher.ENCRYPT_MODE, secretKey);//初始化加密器
        byte[] encryptSign=cipher2.doFinal(signature);//生成加密签名
        privateClientUI.txtArea.append("用密钥加密后的数字签名："+DatatypeConverter.printHexBinary(encryptSign)+"\n\n");         
        //对密钥加密
        cipher.init(Cipher.ENCRYPT_MODE, publicKey);//用服务器公钥初始化加密模式
        byte[] encryptKey=cipher.doFinal(secretKey.getEncoded());//加密密钥 
        privateClientUI.txtArea.append("对密钥加密："+DatatypeConverter.printHexBinary(encryptKey)+"\n\n");
        ...
        out.flush();
        privateClientUI.txtArea.append("3.发送加密的数字签名成功！\n");
        
        //4.发送加密密钥
        out.write(encryptKey);//密文长度为128字节
        out.flush();
        privateClientUI.txtArea.append("4.发送加密的密钥成功！\n");
    ...
    protected void process(List<String> middleResults) {
        for (String str:middleResults) {
            privateClientUI.progressLabel.setText(str);
        }   
    }
    @Override
    protected void done() {
        privateClientUI.progressBar.setValue(privateClientUI.progressBar.getMaximum());
       privateClientUI.txtArea.append(lastResults+"\n");
